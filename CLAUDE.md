...
- All resources subclass `Resource` (`zoko/resources/base.py`), which owns the shared `_request(method, url, *, params, json)`: it sends via a shared `requests.Session`, raises `ZokoError` (`zoko/exceptions.py`) on non-2xx, and returns parsed JSON (or `None` for 204/empty). `Resource._clean(payload)` drops `None` values so optional fields aren't sent as null - every method builds its payload/params dict and passes it through `_clean`.
- `ZokoClient(api_key, max_retries=0, backoff_factor=0.5)` builds the shared `Session`. When `max_retries>0` it mounts an `HTTPAdapter` with a urllib3 `Retry` (status_forcelist `RETRY_STATUSES` = 429/5xx, `allowed_methods=None` so POST retries too, `raise_on_status=False` so the final failing body still surfaces as `ZokoError`). Default 0 = no retries, plain session.
